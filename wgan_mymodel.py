...
parser.add_argument('--output_folder', default='Output/WGAN_mymodel', help='output folder')
...
with h5py.File(h5dataFile, 'r') as h5_data:
    ...
    for epoch in range(param.n_epoch):
        ...
        while t < N_critic :
            ...
            errD_real = torch.mean(D(x).view(-1))
            ...
            errD_fake = torch.mean(D(x_fake).view(-1))
            errD_fake.backward(one_neg)
            
            # Optimize
            errD = torch.mean(errD_real.view(-1)) - torch.mean(errD_fake.view(-1))
            optimizerD.step()
        ...
        x_fake = G(z)
        # Generator Loss
        errG = torch.mean(D(x_fake).view(-1)) 
